...
class SocialGraph:
    ...
    def populate_graph(self, num_users, avg_friendships):
        ...
        # Create friendships based on number of users
    ...
    def get_one_social_path(self, user_id, target_id):

        #Returns all the friends of a node in an array
        def get_neighbors(node_id):
            neighbors = []
            for friend in self.friendships[node_id]:
                neighbors.append(friend)
        ...
        #Create a queue with the user ID in queue
        queue = Queue()
        queue.enqueue([user_id])
        visited = {}  # Note that this is a dictionary, not a set
...
if __name__ == '__main__':
    ...
    #print(connections)
    connection = sg.get_one_social_path(1, 5)
    print(connection)
